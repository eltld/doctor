...
public class Download implements Serializable {
	...
	public Download(String doctorid) {
		// TODO Auto-generated constructor stub
		mLocalPath = Environment.getExternalStorageDirectory()
				.getAbsolutePath()
				+ "/Android/data/net.ememed.doctor2/ememed#ememeduserofficial/"+doctorid;
	}
		...
		try {
			...
			if (httpCode >= 200 && httpCode <= 300) {
				...
				if (!isPause) {
					...
					msg1.what = SUCCESS;
					msg1.obj = mLocalPath  + mFileName;
					System.out.println("图片下载后的路径："+mLocalPath  + mFileName);
	...
	public String getFileName(ChatHistoryEntry entry, String id) {
		...
		if (entry.getTYPE().equals("img")) {
			...
			if (id.equals(entry.getRECEIVER().trim())) {
				...
				filenName = mLocalPath   +"/"+ entry.getCONTENT_EXT().get(0).getFilename();
			}
