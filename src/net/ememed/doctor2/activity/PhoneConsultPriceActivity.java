...
public class PhoneConsultPriceActivity extends BasicActivity {
	...
	protected void setupView() {
		...
		et_setting_price_order.setText(phone_price);
		...
		try {
			new Handler().postDelayed(new Runnable() {
				@Override
				public void run() {
					InputMethodManager inputManager = (InputMethodManager) PhoneConsultPriceActivity.this
...
//					et_setting_price_order.setFocusableInTouchMode(true);
	...
	private void setTextConsultSetting() {
		...
		if (NetWorkUtils.detect(PhoneConsultPriceActivity.this)) {
			loading(null);
			...
			MyApplication.volleyHttpClient.postWithParams(
					HttpUtil.set_phone_consult, ResultInfo.class, params,
		...
		case IResult.SERVICE_SET:// 服务设置
			...
			if (resultInfo != null) {
				if (resultInfo.getSuccess() == 1) {
					...
					event.setPhoneConsultPrice(et_setting_price_order.getText()
							.toString());
					event.setStatus(1);
