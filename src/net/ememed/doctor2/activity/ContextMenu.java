...
public class ContextMenu extends Activity {
	...
	public void copy(View view){
		setResult(DoctorChatFragment.RESULT_CODE_COPY, new Intent().putExtra("position", position));
		finish();
	}
	public void delete(View view){
		setResult(DoctorChatFragment.RESULT_CODE_DELETE, new Intent().putExtra("position", position));
		finish();
	}
	public void forward(View view){
		setResult(DoctorChatFragment.RESULT_CODE_FORWARD, new Intent().putExtra("position", position));
		finish();
	}
	
	public void open(View v){
	    setResult(DoctorChatFragment.RESULT_CODE_OPEN, new Intent().putExtra("position", position));
        finish();
	}
	public void download(View v){
	    setResult(DoctorChatFragment.RESULT_CODE_DWONLOAD, new Intent().putExtra("position", position));
        finish();
	}
	public void toCloud(View v){
	    setResult(DoctorChatFragment.RESULT_CODE_TO_CLOUD, new Intent().putExtra("position", position));
        finish();
	}
