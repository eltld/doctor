 ...
 * @author chen
 * */
public class ServiceZhunyuanSettingActivity extends BasicActivity implements ViewPager.OnPageChangeListener{
	...
	private ZhuyuanSettingEntry setting_detail;
	...
	protected void onCreateContent(Bundle savedInstanceState) {
		...
		setContentView(R.layout.service_in_hospital_setting);
		title = getIntent().getStringExtra("title");
		fromActivity = getIntent().getStringExtra("from");
		setting_detail = (ZhuyuanSettingEntry)getIntent().getSerializableExtra("setting_content");
	}
	...
	protected void setupView() {
		...
		if (setting_detail.getZhuyuan_time_status() == 1) {
			ll_time_setting_detail.setVisibility(View.GONE);
			iv_time_anytime_setting_dot.setEnabled(true);
			iv_time_custom_setting_dot.setEnabled(false);
		} else if (setting_detail.getZhuyuan_time_status() == 2) {
	...
	public void doClick(View view) {
		...
		int id = view.getId();
		if (id == R.id.btn_back) {
			Intent intent = new Intent(ServiceZhunyuanSettingActivity.this,ServiceAllSettingActivity.class);
			intent.putExtra("setting_content", setting_detail);
			intent.putExtra("setting_type", ServiceZhunyuanSettingActivity.class.getSimpleName());
			setResult(RESULT_OK, intent);
			finish();
		} else if (id == R.id.ll_time_anytime_setting) {
			setting_detail.setZhuyuan_time_status(1);
			iv_time_anytime_setting_dot.setEnabled(true);
			iv_time_custom_setting_dot.setEnabled(false);
			ll_time_setting_detail.setVisibility(View.GONE);
		} else if (id == R.id.ll_time_custom_setting) {
			setting_detail.setZhuyuan_time_status(2);
			iv_time_anytime_setting_dot.setEnabled(false);
			iv_time_custom_setting_dot.setEnabled(true);
			ll_time_setting_detail.setVisibility(View.VISIBLE);
	...
	private void setZhuyuan() {
		...
		if (setting_detail.getZhuyuan_time_status() == 2) {//定时
		...
		} else if(setting_detail.getZhuyuan_time_status() == 1) {
		} else {
		...
		params.put("zhuyuan_time_status", setting_detail.getZhuyuan_time_status()+"");

		MyApplication.volleyHttpClient.postWithParams(HttpUtil.set_zhuyuan,
		...
		try {
			...
			case IResult.FINISHED:// 设置开关
				...
				if(success == 0){
					...
					if (setting_detail.getEnable_zhuyuan() == 1) {
						btn_addhealth.setVisibility(View.VISIBLE);
					} else if (setting_detail.getEnable_zhuyuan() == 0) {
			...
			case IResult.SERVICE_SET:// 服务设置
				...
				if(success2 == 0){
					...
					if (!TextUtils.isEmpty(fromActivity) && fromActivity.equals(ServicePackageSettingActivity.class.getSimpleName())) {
						intent = new Intent(ServiceZhunyuanSettingActivity.this,ServicePackageSettingActivity.class);
					} else {
						intent = new Intent(ServiceZhunyuanSettingActivity.this,ServiceAllSettingActivity.class);
					}
					intent.putExtra("setting_content", setting_detail);
					intent.putExtra("setting_type", ServiceZhunyuanSettingActivity.class.getSimpleName());
					setResult(RESULT_OK, intent);
